...
def tiler_batch_meta_handler_cb_1(buffer, user_data):
    ...
    while l_frame is not None:
        ...
        if frame_meta.num_obj_meta > 14:
            ...
            # remove self first
            retval = dsl_tiler_batch_meta_handler_remove('tiler', DSL_PAD_SRC)
            if retval != DSL_RETURN_SUCCESS:
                print(retval)
                return False

            # add second callback
            retval = dsl_tiler_batch_meta_handler_add('tiler', DSL_PAD_SRC, tiler_batch_meta_handler_cb_2, None)
            if retval != DSL_RETURN_SUCCESS:
                print(retval)
                return False
...
def tiler_batch_meta_handler_cb_2(buffer, user_data):
...
def main(args):
    ...
    while True:
        ...
        retval = dsl_tiler_batch_meta_handler_add('tiler', DSL_PAD_SRC, tiler_batch_meta_handler_cb_1, None)
        if retval != DSL_RETURN_SUCCESS:
            break
        ...
        retval = dsl_pipeline_component_add_many('simple-pipeline', 
            ['uri-source', 'primary-gie', 'ktl-tracker', 'tiler', 'on-screen-display', 'overlay-sink', None])
        if retval != DSL_RETURN_SUCCESS:
            break
