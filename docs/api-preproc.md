...
Once added to a Pipeline or Branch, a Preprocess must be removed before it can be used with another.
...
```
The constructor creates a uniquely named Preprocessor. Construction will fail if the name is currently in use.
...
* `enabled` - [in] set to `True` to enable preprocessing, `False` to disabled on creation.
...
**Python Example**
```Python
retval = dsl_preproc_new('my-preprocessor', './my-config-file.txt', True)
```
