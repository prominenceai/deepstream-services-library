# Sink API
Sinks are the end components for a DSL GStreamer Pipeline. A Pipeline must have at least one sink in use, among other components, to reach a state of Ready. DSL supports five types of Sinks:
* Overlay Sink
* Window Sink
* File Sink
* RTSP Sink
...
Sinks are created with five type-specific constructors. As with all components, Sinks must be uniquely named from all other Pipeline components created. 

Sinks are added to a Pipeline by calling [dsl_pipeline_component_add](api-pipeline.md#dsl_pipeline_component_add) or [dsl_pipeline_component_add_many](api-pipeline.md#dsl_pipeline_component_add_many) and removed with [dsl_pipeline_component_remove](api-pipeline.md#dsl_pipeline_component_remove) or [dsl_pipeline_component_remove_many](api-pipeline.md#dsl_pipeline_component_remove_many). 

The relationship between Pipelines and Sinks is one-to-many. Once added to a Pipeline, a Sink must be removed before it can used with another. Sinks are deleted by calling [dsl_component_delete](api-component.md#dsl_component_delete) or [dsl_component_delete_many](api-component.md#dsl_component_delete_many)
...
The maximum number of in-use Sinks is set to `DSL_DEFAULT_SINK_IN_USE_MAX` on DSL initialization. The value can be read by calling [dsl_sink_num_in_use_max_get](#dsl_sink_num_in_use_max_get) and updated with [dsl_sink_set_num_in_use_max](#dsl_sink_set_num_in_use_max). The number of Sources in use by all Pipelines can obtained by calling [dsl_sink_get_num_in_use](#dsl_sink_get_num_in_use). 
...
* [dsl_sink_file_formats_get](#dsl_sink_file_formats_get)
* [dsl_sink_file_formats_set](#dsl_sink_file_formats_set)
* [dsl_sink_file_settings_set](#dsl_sink_file_settings_set)
* [dsl_sink_file_settings_get](#dsl_sink_file_settings_get)
* [dsl_sink_file_settings_set](#dsl_sink_file_settings_set)
...
Note: the server Mounts will be derived from the unique RTSP Sink name, for example: 
```
http://localhost::8050/my-sink-name
```

**Parameters**
* `name` - [in] unique name for the File Sink to create.
* `port` - [in] absolute or relative filespec for the media file to write to.
* `codec` - [in] on of the [Codec Types](#Codec Types) defined above
* `bitrate` - [in] bitrate at which to code the video
* `interval` - [in] frame interval at which to code the video. Set to 0 to code every frame
...
### *dsl_sink_file_formats_get*
This service returns the current codec and container formats for the uniquely named File Sink.
```C++
DslReturnType dsl_sink_file_formats_get(const wchar_t* name, 
...
**Python Example**
```Python
retval, codec, container = dsl_sink_file_formats_get('my-window-sink')
```

<br>

### *dsl_sink_file_formats_set*
...
```C++
DslReturnType dsl_sink_file_formats_set(const wchar_t* name, 
    uint codec, uint container);
```
**Parameters**
* `name` - [in] unique name of the Window Sink to query.
...
**Python Example**
```Python
retval = dsl_sink_file_formats_set('my-window-sink', DSL_CODEC_H265, DSL_CONTAINER_MK4)
```

<br>

### *dsl_sink_file_settings_get*
This service returns the current bitrate and interval settings for the uniquely named File Sink.
```C++
DslReturnType dsl_sink_file_settings_get(const wchar_t* name, 
    uint* bitrate, uint* interval);
```
**Parameters**
* `name` - [in] unique name of the Window Sink to query.
* `bitrate` - [out] current bitrate at which to code the video
* `interval` - [out] current frame interval at which to code the video. Set to 0 to code every frame
...
**Python Example**
```Python
retval, bitrate, interval = dsl_sink_file_settings_get('my-window-sink')
```

<br>

### *dsl_sink_file_settings_set*
...
**Parameters**
* `name` - [in] unique name of the Window Sink to query.
* `bitrate` - [in] the new [Codec Type](#Codec Types) setting to use.
* `interval` - [in] the new  [Video Container Type](#Video Container Types) setting to use.
...
**Python Example**
```Python
retval = dsl_sink_file_settings_set('my-window-sink', 2000000, 1)
```
